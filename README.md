...
- Terminates any in-progress playback from a previous invocation **of the same session and same scope** so a fresh response replaces (not overlaps with) the older one. Single-flight is per-session AND per-scope: the Stop hook (scope `main`) and the mid-turn say.sh (scope `say`) have separate pidfile lanes, so the Stop hook firing at end-of-turn does **not** kill a still-playing mid-turn report. Within the `say` lane the second mid-turn report *waits* for the first to finish instead of preempting it, so consecutive narrations play in order. Concurrent sessions still never silence each other.

...
4. **Write OKF frontmatter at the top of every wiki page** (see schema for controlled `type` vocabulary):
   ...
   description: <one sentence>          # required. See step 5. Keep the value free of
                                        # " #" — YAML would cut it as a comment.
...
10. **Mandatory post-write OKF check.** Run `the `kb-lint` skill's `kb-lint.sh`` and confirm every count on the `summary:` line is 0 — `OKF frontmatter 欠落` / `OKF type 欠落` / `予約構造違反` / `description 欠落` / `description ブロックスカラー` / `description 複数文` / `index 不一致` / `index 書式違反` / `index 行なし` / `index 重複`. Any non-zero count blocks declaring the ingest complete — fix the offending page (add `type:`, write a one-sentence `description`, copy it into the index, or restore the reserved file's required structure) and re-run. Declaring the ingest complete requires steps 8, 9, and 10 to have passed.

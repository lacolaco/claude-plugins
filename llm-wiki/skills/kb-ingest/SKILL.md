...
10. **Mandatory post-write OKF check.** Run `the `kb-lint` skill's `kb-lint.sh`` and confirm the summary reports `frontmatter 欠落 0 / type 欠落 0 / 予約構造違反 0 / description 欠落 0 / description 複数文 0 / index 不一致 0 / index 書式違反 0`. Any non-zero count blocks declaring the ingest complete — fix the offending page (add `type:`, write a one-sentence `description`, copy it into the index, or restore the reserved file's required structure) and re-run. Declaring the ingest complete requires steps 8, 9, and 10 to have passed.

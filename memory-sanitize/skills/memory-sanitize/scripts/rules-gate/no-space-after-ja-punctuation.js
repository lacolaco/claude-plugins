// memory-sanitize 独自 rule: 句読点の直後に半角スペースを入れない
//
...
//   - Markdown のハード改行 (行末の半角スペース 2 つ)。remark はこの 2 つを
//     Str の生テキストに残さず Break ノードの構文として消費するため、
//     「句点 + 半角スペース 2 つ + 改行」は原理的に Str の走査で検出できない。
//     規範文書でハード改行は使わないので実害はないが、盲点として記録する
//
    ...
    [Syntax.Str](node) {
      ...
      while ((m = AFTER_PUNCT_RE.exec(raw)) !== null) {
        report(
          node,
          new RuleError(
            '原則として、句読点の直後に半角スペースを入れません: 英文の書式を日本語に持ち込んでいる',

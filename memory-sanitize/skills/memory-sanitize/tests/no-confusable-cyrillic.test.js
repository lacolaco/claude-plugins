...
tester.run('no-confusable-cyrillic', rule, {
  ...
  invalid: [
    ...
    {
      // コードブロックも対象。位置も固定する。getSource(node) が返す文字列の
      // 起点と locator.range() の基準点はどちらも node.range[0] なので、
      // フェンス記号を含んだまま計算しても報告位置はずれない。この対応関係が
      // 崩れる変更を検知するため、メッセージだけでなく行と桁を主張する。
      text: '```\nсd /tmp\n```',
      errors: [{
